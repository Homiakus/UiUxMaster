...
func (e *fmea007EffectExecutor) StepPolish(ctx context.Context, _ DesignPolishRequest, iter int) (PolishIteration, error) {
	...
	sourceOp := sideeffect.Operation{RunID: identity.ExecutionID, Activity: identity.NodeID, Iteration: iter, Kind: "source_repair", PayloadDigest: desired.Digest(), RetryClass: sideeffect.RetryIdempotent}
	...
	sourceNS, _ := memory.NewProjectEvidenceNamespace("fmea007")
	prov := memory.ProvenanceRecord{RunID: identity.ExecutionID, EvidenceDigest: "sha256:fmea007", Renderer: "playwright", ProjectScope: "fmea007", SourceNamespace: sourceNS.String(), Timestamp: time.Now()}
	...
	memoryOp := sideeffect.Operation{RunID: identity.ExecutionID, Activity: identity.NodeID, Iteration: iter, Kind: "memory_admission", PayloadDigest: memoryPayload, RetryClass: sideeffect.RetryIdempotent}
...
func TestFMEA007AxiomRetryReusesTargetEffectsAndStableActivityKey(t *testing.T) {
	...
	if !executor.receipts[2].Reused || executor.receipts[2].Applied || !executor.receipts[3].Reused || executor.receipts[3].Applied { t.Fatalf("retry receipts=%#v", executor.receipts[2:]) }
	if len(executor.activityKeys) != 2 || executor.activityKeys[0] == "" || executor.activityKeys[0] != executor.activityKeys[1] { t.Fatalf("activity keys=%v", executor.activityKeys) }

	ns, _ := memory.NewProjectKnowledgeNamespace("fmea007")
	edges, err := executor.memory.GetEdges(ctx, ns, "pattern:fmea007")
	if err != nil { t.Fatal(err) }
