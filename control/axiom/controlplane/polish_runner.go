...
func newPolishRunner(executor DesignPolishExecutor, store adgo.Store) (*PolishRunner, error) {
	if executor == nil {
		return nil, fmt.Errorf("controlplane: polish executor is required")
	}
	plan, err := adgo.Compile(polishDefinition())
	if err != nil {
		return nil, fmt.Errorf("controlplane: compile polish plan: %w", err)
	}
...
func polishDefinition() adgo.Definition {
	return adgo.Definition{
		ID:          polishPlanID,
		Version:     polishPlanVersion,
		InitialData: []string{dataPolishRequest},
		Nodes: []adgo.Node{
			{
				ID:       "inspect_baseline",
				Kind:     adgo.NodeActivity,
				Activity: activityPolishInspect,
				Requires: []string{dataPolishRequest},
				Produces: []string{dataPolishBaseline},
				Next:     []adgo.Transition{{To: "iterate_repair"}},
			},
			{
				ID:        "iterate_repair",
				Kind:      adgo.NodeActivity,
				Activity:  activityPolishIterate,
				DependsOn: []string{"inspect_baseline"},
				Requires:  []string{dataPolishRequest, dataPolishBaseline},
				Produces:  []string{dataPolishIterations},
				Next:      []adgo.Transition{{To: "conclude_polish"}},
			},
			{
				ID:        "conclude_polish",
				Kind:      adgo.NodeActivity,
				Activity:  activityPolishConclude,
				DependsOn: []string{"iterate_repair"},
				Requires:  []string{dataPolishRequest, dataPolishIterations},
				Produces:  []string{dataPolishResult},
			},
		},
	}
}
...
func registerPolishActivities(registry *adgo.Registry, executor DesignPolishExecutor) {
	registry.Activity(activityPolishInspect, func(ctx context.Context, req adgo.ActivityRequest) (adgo.ActivityResult, error) {
		...
		return adgo.ActivityResult{
			Facts:   map[string]any{dataPolishBaseline: iter},
	...
	registry.Activity(activityPolishIterate, func(ctx context.Context, req adgo.ActivityRequest) (adgo.ActivityResult, error) {
		...
		for i := 1; i <= maxIters; i++ {
			...
			if iter.HardViolations == 0 && iter.FindingsCount == 0 {
				break // converged!
			}
		}

		return adgo.ActivityResult{
			Facts:   map[string]any{dataPolishIterations: iters},
			Outcome: adgo.OutcomeCompleted,
		}, nil
	})
	...
	registry.Activity(activityPolishConclude, func(ctx context.Context, req adgo.ActivityRequest) (adgo.ActivityResult, error) {
		...
		return adgo.ActivityResult{
			Facts:   map[string]any{dataPolishResult: res},
			Outcome: adgo.OutcomeCompleted,
		}, nil
	})
}
...
func (r *PolishRunner) StartAndRun(ctx context.Context, id string, req DesignPolishRequest, budget Budget) (DesignPolishRun, error) {
	if r == nil || r.runtime == nil {
		return DesignPolishRun{}, fmt.Errorf("controlplane: polish runner is not initialized")
	}
	exec, err := r.runtime.Start(ctx, id, map[string]any{dataPolishRequest: req}, toAxiomBudget(budget))
	if err != nil {
		return DesignPolishRun{}, err
	}
...
func projectPolishExecution(execution *adgo.Execution) (DesignPolishRun, error) {
	if execution == nil {
		return DesignPolishRun{}, fmt.Errorf("controlplane: nil execution")
	}
