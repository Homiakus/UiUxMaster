...
// EpMemoryStore is an in-process conflict-preserving epistemic store. FMEA-007
// adds two independent idempotency barriers: operation receipts prevent replay of
// the same admitted transaction, while semantic edge/conflict keys prevent graph
// multiplicity even for legacy Commit callers.
...
// Commit remains source-compatible but is now semantically idempotent for atoms,
// edges and conflict links. Durable/retried workflow effects should prefer
// CommitOnce so a stable operation receipt is also available for audit/replay.
...
// CommitOnce binds one memory-admission side effect to a stable operation ID.
// Replaying the same operation/payload returns the original result without
// mutating atoms, edges or conflicts. Reusing an operation ID for another payload
// fails closed.
func (s *EpMemoryStore) CommitOnce(ctx context.Context, op sideeffect.Operation, bundle AdmissionBundle) (sideeffect.Receipt, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	if prior, ok := s.committedOperations[opID]; ok {
		if prior.PayloadDigest != op.PayloadDigest {
	...
	s.committedOperations[opID] = receipt
	return receipt, nil
}

func (s *EpMemoryStore) OperationReceipt(operationID string) (sideeffect.Receipt, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	r, ok := s.committedOperations[operationID]
	return r, ok
}
...
func (s *EpMemoryStore) addEdgeLocked(edge MemoryEdge, now time.Time) {
	...
	if _, exists := s.conflictKeys[conflictKey]; !exists {
		...
		s.conflicts = append(s.conflicts, ConflictRecord{
			PrimaryAtomID: edge.ToID, ConflictingAtomID: edge.FromID,
			Reason: fmt.Sprintf("Relation %s asserted from %s", edge.Relation, edge.FromID), DetectedAt: now,
		})
	}
...
func memoryBundleResultDigest(bundle AdmissionBundle) string {
	...
	for _, atom := range bundle.Atoms {
		parts = append(parts, "a:"+atom.ID)
	}
	for _, edge := range bundle.Edges {
		parts = append(parts, "e:"+memoryEdgeKey(edge))
	}
	sort.Strings(parts)
...
func (s *EpMemoryStore) Supersede(ctx context.Context, oldAtomID string, newAtom MemoryAtom, prov ProvenanceRecord) error {
	...
	s.addEdgeLocked(MemoryEdge{FromID: newAtom.ID, ToID: oldAtomID, Relation: RelGeneralizes, Weight: 1.0, Provenance: prov, CreatedAt: now}, now)
	return nil
}
...
func (s *EpMemoryStore) Query(ctx context.Context, q QueryRequest) (*QueryResult, error) {
	...
	for _, stored := range s.atoms {
		...
		if len(q.Tags) > 0 {
			...
			for _, t := range atom.Tags { tagMap[t] = true }
			matchAll := true
			for _, reqTag := range q.Tags { if !tagMap[reqTag] { matchAll = false; break } }
			if !matchAll { continue }
		}
		matches = append(matches, atom)
	}
	sort.Slice(matches, func(i, j int) bool {
		if matches[i].Confidence != matches[j].Confidence { return matches[i].Confidence > matches[j].Confidence }
		return matches[i].CreatedAt.After(matches[j].CreatedAt)
	})
	total := len(matches)
	if q.Limit > 0 && len(matches) > q.Limit { matches = matches[:q.Limit] }
