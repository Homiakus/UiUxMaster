...
func (c *CDPCollector) CollectL2(ctx context.Context, req engine.ValidationRequest, plan evidenceplan.Plan) (evidence.Packet, error) {
	...
	cdpReq := fastcdp.RequestFromPlan(plan, fastcdp.PlannedRequestOptions{
		...
		ExpectedRevision:   strings.TrimSpace(req.SourceDigest),
	...
	packet := fastcdp.ToPacket(collected, fastcdp.PacketOptions{
		...
		ExpectedRevision: cdpReq.ExpectedRevision,
	})
	...
	if len(req.Scope.Routes) > 0 {
		...
		for _, route := range req.Scope.Routes {
			...
			if !exists {
				packet.Documents = append(packet.Documents, evidence.DocumentMetrics{URL: route})
			}
		}
	}
	...
	for _, rStr := range req.Scope.Regions {
		...
		if !exists {
			var b evidence.Rect
			if parsed, ok := parseRegionBounds(rStr); ok {
				b = evidence.Rect{X: parsed.X, Y: parsed.Y, Width: parsed.Width, Height: parsed.Height}
			}
			packet.VisualRegions = append(packet.VisualRegions, evidence.VisualRegion{ID: regionID, Bounds: b})
		}
	}
	...
	if cdpReq.Region != nil && packet.Pixels != nil {
		...
		if !hasRequestedROI {
			packet.VisualRegions = append(packet.VisualRegions, evidence.VisualRegion{ID: "requested-roi", Bounds: packet.Pixels.Bounds})
		}
	}
...
func parseRegionBounds(raw string) (*evidenceplan.Region, bool) {
	...
	return &evidenceplan.Region{X: x, Y: y, Width: w, Height: h, Scale: 1}, true
}
