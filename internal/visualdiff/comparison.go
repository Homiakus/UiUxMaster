...
// baseline environment key, making churn/flakiness visible per environment
// instead of blending unrelated browser/runtime populations.
...
func NewComparator() *Comparator {
	return &Comparator{metrics: make(map[string]ComparisonMetrics), outcomes: make(map[string]string)}
}
...
func (c *Comparator) CompareBaseline(req ComparisonRequest) (ComparisonResult, error) {
	...
	baselineDigest := req.Baseline.DigestSHA256
	if baselineDigest == "" { baselineDigest = imageDigest(req.BaselineImage) }
...
func validateMaskOwnership(masks []evidence.DynamicMask, elements []evidence.ElementRef, imageBounds image.Rectangle) ([]image.Rectangle, error) {
	...
	for _, mask := range masks {
		...
		r := image.Rect(int(mask.Bounds.X), int(mask.Bounds.Y), int(mask.Bounds.X+mask.Bounds.Width), int(mask.Bounds.Y+mask.Bounds.Height))
...
func rectContains(owner, child evidence.Rect) bool {
	return child.X >= owner.X && child.Y >= owner.Y &&
		child.X+child.Width <= owner.X+owner.Width && child.Y+child.Height <= owner.Y+owner.Height
}
...
func digestComparison(environmentKey, baselineDigest, candidateDigest string, opts Options, masks []evidence.DynamicMask, result Result) string {
	...
	for _, mask := range masks {
		maskIDs = append(maskIDs, fmt.Sprintf("%s:%s:%.3f,%.3f,%.3f,%.3f", mask.ID, mask.OwnerElementID, mask.Bounds.X, mask.Bounds.Y, mask.Bounds.Width, mask.Bounds.Height))
	}
	sort.Strings(maskIDs)
	payload := fmt.Sprintf("%s\x00%s\x00%s\x00tol=%d\x00masks=%s\x00changed=%d\x00ratio=%.12f\x00max=%d",
		environmentKey, baselineDigest, candidateDigest, opts.ChannelTolerance, strings.Join(maskIDs, "|"), result.ChangedPixels, result.ChangeRatio, result.MaxDelta)
