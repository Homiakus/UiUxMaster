...
func TestFMEA007SourceCASReplayAfterRestartIsExactlyOnce(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if !first.Applied || first.Reused { t.Fatalf("first receipt=%#v", first) }

	// Simulate a process crash immediately after the durable side effect: reopen
	// from disk before the workflow can persist activity completion.
	restarted, err := NewFileSourceStore(path, SourceState{})
	if err != nil { t.Fatal(err) }
	second, err := restarted.CompareAndSwap(ctx, op, initial.Digest(), desired)
	if err != nil { t.Fatal(err) }
	if second.Applied || !second.Reused || second.OperationID != first.OperationID || second.ResultDigest != first.ResultDigest {
...
func TestFMEA007OperationIDRejectsPayloadMutation(t *testing.T) {
	store := NewMemorySourceStore(SourceState{HTML: "a"})
	ctx := context.Background()
	desired := SourceState{HTML: "b"}
	op := Operation{RunID: "run", Activity: "repair", Kind: "source_repair", PayloadDigest: desired.Digest(), RetryClass: RetryIdempotent}
	if _, err := store.CompareAndSwap(ctx, op, SourceState{HTML: "a"}.Digest(), desired); err != nil { t.Fatal(err) }
	// Same operation coordinates but a mutated payload produces a different stable
	// operation ID; because the expected revision is stale, CAS refuses overwrite.
	...
	if !errors.Is(err, ErrCASConflict) { t.Fatalf("err=%v want CAS conflict", err) }
}
